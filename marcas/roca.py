...
class Roca:
    ...
    def excel_reader(self):
        ...
        dfincepa['Referencias'] = dfincepa['Referencias'].apply(lambda x: self.ajuste_referencia_sku(x))
        dfincepa = dfincepa[['Referencias','Nome','Saldo0']]
        incepa = self.df_hausz('Roca')
        hauszdf = pd.DataFrame(incepa)
      
        finall = pd.merge(hauszdf, dfincepa, left_on='Sku', right_on='Referencias', how='left')
